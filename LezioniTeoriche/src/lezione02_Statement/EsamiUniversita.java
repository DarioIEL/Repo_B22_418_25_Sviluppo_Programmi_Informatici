...
public class EsamiUniversita {
	public static void main(String[] args) {
		 ...
		 * singolo esame alla volta. L'esame scritto farà da sogli adi sbarramento
		 * all'esame orale (il mio voto scritto deve essere di almeno 18). Fornisci la
		 * possibilità all'utente di inserire i voti uno alla volta. Quando sono
		 * inseriti entrambi i voti calcola il voto finale (media dei due)
		 */
			...
			if (votoScritto >= SOGLIA) {
				...
				if (votoOrale <= VOTO_MAX && votoOrale >= VOTO_MIN) {
					...
					if (votoOrale >= 18) {
						...
						// Esame non superato
						System.out.println(
								"Mi spiace, non hai superato l'esame orale. Quindi no hai superato l'intero esame");
					}
